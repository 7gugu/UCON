...
$rom=mysql_fetch_array($rs);
	...
	//header("Location: http://localhost/ace/uphp/index.php");
	echo "验证失败";
}
}else{
	echo "验证失败";
	...
	$query= "SELECT map,sport,sename,dtime,max,rtime FROM op WHERE username='{$name}'";
	...
	<body>
		<div class="container">
			...
			<address>
				 ...
				 <?php
                 if($rtime=="1"){$sgame="运行中...";}else{$sgame="停止中...";}				 
				 echo $sgame;
				 ?> 
				 <br /> 到期时间:<?php echo $dtime."天";?><br />  
			</address>
		</div>
		<div class="col-md-6">
			...
			<input type=hidden name="order" id="order" value="shutdown">
			<button type="sumbit"  class="btn btn-danger btn-block <?php if($rtime=="0"){echo "disabled";}?>">
				Stop
			</button>
			</form>
		</div>
